<?php
	...
	switch( $action ){
		...
		case 'addImage':    addImage();
            break;
        case 'updateGroupImage':   updateGroupImage();
            break;
        case 'createGroup':createGroup();
			break;			
		case 'acceptGroupRequest':acceptGroupRequest();
			break;	
		case 'declineGroupRequest':declineGroupRequest();
			break;	
        case 'acceptChatRequest':acceptChatRequest();
			break;	
		case 'declineChatRequest':	declineChatRequest();
			break;
        case 'acceptDateRequest':acceptDateRequest();
			break;	
		case 'declineDateRequest':declineDateRequest();
			break;			
		case 'assignGroup':assignGroup();
			break;					
		case 'profileSearch':profileSearch();
			break;
		case 'dateForm':	dateForm();
			break;
		case 'viewProfile':	viewProfile();
			break;
		case 'viewGroup':	viewGroup();
			break;
		case 'createChatGroup':	createChatGroup();
			break;
		 case 'updateChatGroupImage':   updateChatGroupImage();
            break;	
	    case 'chat':	chat();
			break;
		case 'dateRequest':	dateRequest();
			break;	
		case 'loadMessage':	loadMessage();
			break;
		case 'loadMessageByUser':	loadMessageByUser();
			break;
		case 'shout':shout();
			break;
		...
		case 'addAnswer':	addAnswer();
			break;
		case 'addShop':	addShop();
			break;	
		case 'idUpload':	idUpload();
			break;	
		case 'updateForm':     updateForm();
			break;
		case 'updateUserImage':     updateUserImage();
			break;
		default:	login(); 
	}
		 ...
		 if( !isset( $_SESSION['email'] ) ){
			 if(isset($_POST['email']))
			 {
				...
				if( $user = User::getByEmail( $email ) ){
					...
					header( "Location: ../search.php" );     // if its a verified account goes to home
					}
	...
	function viewProfile(){
		if(isset($_SESSION['personId']))														// got id .. load info acc to it
        header("Location: ../profile.php" );  
	}
	
	function viewGroup(){
	$_SESSION['groupId']=$_GET['id'];
		if(isset($_SESSION['groupId']))																// got id .. load info acc to it
...
$sql_con = mysqli_connect($db_host, $db_username, $db_password,$db_name)or die('could not connect to database');
	...
	function loadMessageByUser(){				// message load by user ????????
	   ...
	   $_SESSION['chatId']=$_POST['userId'];
		header('Location:main.php?action=chat');
	...
	function logout(){
		...
		header("Location: /index.php");     // main land page specifically.
	}
...
if($_POST)
{
    ...
    function createGroup(){
	 ...
	 function createChatGroup(){
		$results = array();	
		$results['pageTitle'] = " Add an activity | Dating website";
		if(isset($_SESSION['mygroupId']))
		{
		$chatGroup = new ChatGroup( $_POST );
		$chatGroup->group1Id = $_SESSION['mygroupId'];
		$chatGroup->icon_link = DEFAULT_IMAGE_LOCATION;
        $chatGroup->status = "pending";
		$group2 = Group::getById($_SESSION['groupId']);
		...
		//require( TEMPLATE_PATH . "/home.php" );
		header('Location:main.php?action=viewProfile');
	...
	function dateRequest(){     // hangout request
		...
		$results['pageTitle'] = " Add an activity | Dating website";
			...
			//header('Location: ' . $_SERVER['HTTP_REFERER']);
		    header('Location:main.php?action=dateForm');
